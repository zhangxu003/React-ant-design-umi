...
/* Show the Adobe Express Install dialog
   ...
   */
let showExpressInstall = (att, par, replaceElemIdStr, callbackFn) => {
...
let createSWF = (attObj, parObj, id) => {
  var r,
...
let createObjParam = (el, pName, pValue) => {
...
/* Updated attachEvent function for Internet Explorer
   ...
   */
let addListener = (target, eventType, fn) => {
  target.attachEvent(eventType, fn);
  listenersArr[listenersArr.length] = [target, eventType, fn];
};

/* Flash Player and SWF content version matching */
let hasPlayerVersion = rv => {
  var pv = ua.pv,
    v = rv.split('.');
  v[0] = parseInt(v[0], 10);
...
/* Cross-browser dynamic CSS creation
   ...
   */
let createCSS = (sel, decl, media, newStyle) => {
  if (ua.ie && ua.mac) {
    return;
  }
...
let setVisibility = (id, isVisible) => {
  if (!autoHideShow) {
    return;
  }
