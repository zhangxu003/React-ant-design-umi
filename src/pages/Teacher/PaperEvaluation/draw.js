...
import { Radio, Divider, Tooltip } from 'antd';
import cs from 'classnames';
import styles from './draw.less';

const toolList = [
  { key: 'frame', icon: 'icon-board-rectangle', title: '添加方框' },
  { key: 'circle', icon: 'icon-board-circular', title: '添加椭圆' },
  { key: 'arrow', icon: 'icon-board-arrow', title: '添加箭头' },
  { key: 'pen', icon: 'icon-board-pen', title: '画笔工具' },
  { key: 'text', icon: 'icon-board-text', title: '添加文字' },
];
...
class Draw extends PureComponent {
  state = {
    ...
    size: 'middle', // 字体大小
    type: 'show', // 绘图模式，draw: 画面模式； show: 试卷模式
  };
  ...
  componentDidMount() {
    ...
    // 监听高度变化，重新给canvas赋值高度
    const observer = new MutationObserver(() => {
      this.resetCanvasAttr();
    });
    observer.observe(relation.current, {
  ...
  toolRender = () => {
    ...
    return toolList.map(item => (
      <Tooltip title={item.title} overlayClassName={styles.tooltip}>
        <div
          key={item.key}
          className={cs(
